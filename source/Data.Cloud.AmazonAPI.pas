...
type
  ...
  public
    ...
    function BuildAuthorizationString(const StringToSign, DateISO, Region, SignedStrHeaders, ServiceName: string): string; reintroduce; overload;
    function BuildAuthorizationString(const StringToSign, ServiceName: string; const QueryParameters: TStringList): string; reintroduce; overload;
  end;
  ...
  private
    ...
    procedure SetStorageEndpoint(const endpoint: string);
  public
    ...
    /// <param name="BucketName">The optional bucket name to prefix the host path</param>
  ...
  TAmazonRegion = (amzrNotSpecified, amzrEUWest1, amzrEUCentral1, amzrUSEast1, amzrUSWest1, amzrUSWest2,
                   amzrAPSoutheast1, amzrAPSoutheast2, amzrAPNortheast1, amzrSAEast1, amzrUSClassic, amzrEU, amzrBeijing);
  ...
  private
    ...
    function GetBucketXMLInternal(const BucketName: string; OptionalParams: TStrings; VersionInfo: Boolean;
                                  ResponseInfo: TCloudResponseInfo; BucketRegion: TAmazonRegion = amzrNotSpecified): string;
    ...
    function GetEndpointFromRegion(Region: TAmazonRegion): string;
    function GetVirtualHostFromRegion(const bucketname: string; location: TAmazonRegion): string;
  protected
  ...
  public
    ...
    function GetBucketXML(const BucketName: string; OptionalParams: TStrings;
                          ResponseInfo: TCloudResponseInfo = nil; BucketRegion: TAmazonRegion = amzrNotSpecified): string;
    /// <summary>Returns some or all of the objects of a given bucket.</summary>
    ...
    function GetBucket(const BucketName: string; OptionalParams: TStrings;
                       ResponseInfo: TCloudResponseInfo = nil; BucketRegion: TAmazonRegion = amzrNotSpecified): TAmazonBucketResult;
    ...
    function SetBucketNotification(const BucketName: string; Events: TList<TAmazonNotificationEvent>;
                                   ResponseInfo: TCloudResponseInfo = nil; BucketRegion: TAmazonRegion = amzrNotSpecified): Boolean;
...
uses
  ...
  System.TypInfo, System.DateUtils, Xml.XMLDoc, System.StrUtils, System.NetEncoding, System.Net.URLClient
  {$IFDEF MACOS}
...
function TAmazonAWS4Authentication.GetSignatureKey(const datestamp, region, ServiceName: string): TBytes;
begin
...
function TAmazonAWS4Authentication.BuildAuthorizationString(const StringToSign, DateISO, Region, SignedStrHeaders, ServiceName: string): string;
var
...
function TAmazonAWS4Authentication.BuildAuthorizationString(const StringToSign, ServiceName: string; const QueryParameters: TStringList): string;
var
...
begin
  ...
  Result := QueryParameters.Values['X-Amz-Algorithm'] + #10 + LdateISO
    + #10 +Ldate + '/' + Lregion + '/' + ServiceName + '/aws4_request';
...
procedure TAmazonConnectionInfo.SetStorageEndpoint(const endpoint: string);
begin
   FStorageEndpoint := endpoint;
   if endpoint = DefaultStorageEndpoint then
      UseDefaultEndpoints := true
   else
...
function TAmazonConnectionInfo.VirtualHost(const BucketName: string): string;
begin
  if BucketName = EmptyStr then
    Result := StorageEndpoint
  else
    Result := Format('%s.%s', [BucketName, StorageEndpoint]);
end;
...
procedure TAmazonQueueService.PrepareRequestSignature(const HTTPVerb: string;
                                  const Headers, QueryParameters: TStringList;
                                  const StringToSign: string;
...
begin
  ...
  begin
    ...
    SigningKey := TAmazonAWS4Authentication(FAuthenticator).GetSignatureKey(LDateISO, 'cn-north-1', 'sqs');

    AuthorizationString := THash.DigestAsString(TAmazonAWS4Authentication(FAuthenticator).SignString(SigningKey, StringToSign));
...
function TAmazonQueueService.BuildStringToSign(const HTTPVerb: string; Headers, QueryParameters: TStringList;
...
begin
  ...
  //Host
  Result := Result + #10'host:sqs.cn-north-1.amazonaws.com.cn'#10;
  Result := Result + #10'host';
  Result := Result + #10'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855';


  Result := TAmazonAWS4Authentication(FAuthenticator).BuildAuthorizationString(Result, 'sqs', QueryParameters);
...
begin
  ...
  Ldate :=  Leftstr(LdateISO,8);
  Lregion := 'cn-north-1';
  Scope := ConnectionInfo.AccountName + '/' + Ldate + '/'+Lregion+ '/sqs' + '/aws4_request';
...
//  Result.Values['X-Amz-Signature'] := 'host';
//
...
begin
  Result := TAmazonAWS4Authentication.Create(ConnectionInfo, False); //authentication isn't put in header
end;
...
begin
  ...
  begin
    ...
    LdateISO :=  Leftstr(Headers.Values['x-amz-date'],8);
    Lregion := GetRegionFromEndpoint(Headers.Values['Host']);
    AuthorizationString := TAmazonAWS4Authentication(FAuthenticator).BuildAuthorizationString(StringToSign, LdateISO, Lregion, SignStrHeaders, 's3');
    Request.Client.CustomHeaders['Authorization'] :=  AuthorizationString;
    SignedHeaders.Clear;
    if RequiredHeadersInstanceOwner then
...
var
  LStartIdx: Integer;
begin
  if Endpoint.Contains('s3.amazonaws.com') or Endpoint.Contains('s3-external-1.amazonaws.com') then
     Exit('us-east-1');
  if Endpoint.Contains('s3-us-west-2.amazonaws.com') then
     Exit('us-west-2');
  if Endpoint.Contains('s3-us-west-1.amazonaws.com') then
     Exit('us-west-1');
  if Endpoint.Contains('s3-eu-west-1.amazonaws.com') then
     Exit('eu-west-1');
  if Endpoint.Contains('s3-eu-central-1.amazonaws.com') or Endpoint.Contains('s3.eu-central-1.amazonaws.com') then
     Exit('eu-central-1');
  if Endpoint.Contains('s3-ap-southeast-1.amazonaws.com') then
     Exit('ap-southeast-1');
  if Endpoint.Contains('s3-ap-southeast-2.amazonaws.com') then
     Exit('ap-southeast-2');
  if Endpoint.Contains('s3-ap-northeast-1.amazonaws.com') then
     Exit('ap-northeast-1');
  if Endpoint.Contains('s3-sa-east-1.amazonaws.com') then
     Exit('sa-east-1');
  if Endpoint.Contains('s3.cn-north-1.amazonaws.com.cn') then
     Exit('cn-north-1')
  else
  begin
    if Endpoint.EndsWith('.amazonaws.com') then
    begin
...
begin
  if Region.Equals('EU') then
    Exit(amzrEUWest1);
  if Region.Equals('eu-west-1') then
    Exit(amzrEUWest1);
  if Region.Equals('eu-central-1') then
    Exit(amzrEUCentral1);
  if Region.Equals('us-east-1') then
    Exit(amzrUSEast1);
  if Region.Equals('us-west-1') then
    Exit(amzrUSWest1);
  if Region.Equals( 'us-west-2') then
    Exit(amzrUSWest2);
  if Region.Equals('ap-southeast-1') then
    Exit(amzrAPSoutheast1);
  if Region.Equals('ap-southeast-2') then
    Exit(amzrAPSoutheast2);
  if Region.Equals('ap-northeast-1') then
    Exit(amzrAPNortheast1);
  if Region.Equals('sa-east-1') then
    Exit(amzrSAEast1);
...
//note: endpoints and regions may change in the future
function TAmazonStorageService.GetEndpointFromRegion(Region: TAmazonRegion): string;
begin
  case Region of
    amzrEU: Result := 's3-eu-west-1.amazonaws.com'; // deprecate
    amzrEUWest1: Result := 's3-eu-west-1.amazonaws.com';
    amzrEUCentral1: Result := 's3-eu-central-1.amazonaws.com';
    amzrUSEast1: Result := 's3.amazonaws.com';  //s3-external-1.amazonaws.com (N. Virginia only)
    amzrUSWest1: Result := 's3-us-west-1.amazonaws.com';
    amzrUSWest2: Result := 's3-us-west-2.amazonaws.com';
    amzrAPSoutheast1: Result := 's3-ap-southeast-1.amazonaws.com';
    amzrAPSoutheast2: Result := 's3-ap-southeast-2.amazonaws.com';
    amzrAPNortheast1: Result := 's3-ap-northeast-1.amazonaws.com';
    amzrSAEast1: Result := 's3-sa-east-1.amazonaws.com';
    amzrNotSpecified: Result := 'us-east-1'; // deprecate
    amzrUSClassic: Result := 's3.amazonaws.com'; // deprecate
    amzrBeijing: Result :='s3.cn-north-1.amazonaws.com.cn';
  end;
end;
...
function TAmazonStorageService.GetVirtualHostFromRegion(const bucketname: string; location: TAmazonRegion): string;
begin
  if location in [amzrUSEast1, amzrUSClassic, amzrNotSpecified] then
    Result := Format('%s.%s', [bucketname, GetConnectionInfo.DefaultStorageEndpoint])
  else
    result := Format('%s.%s', [bucketname, GetEndpointFromRegion(location)]);
end;
...
function TAmazonStorageService.InitHeaders(const BucketName: string): TStringList;
begin
  Result := TStringList.Create;
...
begin
  ...
  begin
    RegionStr := GetRegionFromEndpoint(GetConnectionInfo.GetStorageEndpoint);  //create bucket in same region as endpoint
    virtualhost:= BucketName + '.' + GetConnectionInfo.FStorageEndpoint;
    url := GetConnectionInfo.GetServiceURL(virtualhost) + '/';
...
begin
  ...
  try
    ...
    if not GetRegionFromEndpoint(GetConnectionInfo.GetStorageEndpoint).Equals(GetRegionString(BucketRegion)) then
    begin
...
begin
  ...
  try
    ...
    begin
      virtualhost := BucketName + '.' + GetConnectionInfo.FStorageEndpoint;
      url := GetConnectionInfo.GetServiceURL(virtualhost)+ '/?notification=';
      Headers.Values['host'] := virtualhost;
    end else
    if not GetRegionFromEndpoint(GetConnectionInfo.GetStorageEndpoint).Equals(GetRegionString(BucketRegion)) then
    begin
...
function TAmazonStorageService.GetBucketNotification(const BucketName: string;
                                      ResponseInfo: TCloudResponseInfo;
                                      BucketRegion: TAmazonRegion): TList<TAmazonNotificationEvent>;
var
  xml: string;
  xmlDoc: IXMLDocument;
...
function TAmazonStorageService.SetBucketLifecycle(const ABucketName: string;
...
function TAmazonStorageService.DeleteBucketLifecycle(const ABucketName: string;
  const AResponseInfo: TCloudResponseInfo): Boolean;
var
  LUrl: string;
  LHeaders: TStringList;
